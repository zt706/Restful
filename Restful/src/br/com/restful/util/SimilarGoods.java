...
public class SimilarGoods
{
	...
	private  static final String QUERYSQL = "select *"
			+ " from recom_product_similar_product"
			+ " where site_id = 1"
			+ " and status = 1"
			+ " and goodsid = %s";
	
	public static String getSimilarGoods(Properties props, long goodsId, String filterIds)
	{
		// 将全站统一过滤id加入过滤ids
		...
		if (web_filter_str != null && web_filter_str.length() > 0)
		{
		 ...
		 String query_sql = String.format(QUERYSQL, goodsId);
